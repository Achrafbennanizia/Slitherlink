# Slitherlink Solver (Backup Archive)

Backup copy of the Slitherlink solver. Production code is TBB-based (V10/main.cpp in the main repo); historical std::async versions (V1–V9) live here under `old_versions/` for reference.
...
└── old_versions/           # Historical code snapshots
...
**Version reality:** V1–V9 are std::async-only; V10 is the first and only TBB rewrite (basis for main.cpp). If an older note suggests gradual TBB integration before V10, treat it as historical commentary rather than code. For full documentation, refer to the main repository `docs/` and `COMPLETE_CODE_HISTORY.md`.
