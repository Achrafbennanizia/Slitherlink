...
struct Solver
{
    ...
    int maxParallelDepth = 10;
    int maxThreads = max(16, (int)thread::hardware_concurrency() * 2);
    ...
    int selectNextEdge(const State &s, int startIdx) const
    {
        ...
        // First pass: look for critical edges only
        for (int i = 0; i < (int)edges.size(); ++i)
        {
            ...
            if (e.cellA >= 0 && grid.clues[e.cellA] >= 0)
            {
                ...
                if (undecided > 0)
                {
                    ...
                    if (needed == undecided)
                        score += 400; // All remaining must be on
                    else if (needed == 0)
                        score += 400; // All remaining must be off
                    else if (undecided <= 2)
                        score += 200; // Few choices left
                    else
                        score += max(0, 50 - abs(needed * 2 - undecided));
                }
            }
    ...
    void solveRecursivePtr(unique_ptr<State> s, int edgeIdx, int depth)
    {
        ...
        // Quick validity check before expensive propagation
        ...
        // Quick test: can this edge be on or off?
        bool canBeOff = true;
        bool canBeOn = true;

        // Test OFF branch cheaply
        {
            int deg_u = s->pointDegree[edges[edgeIdx].u];
            int deg_v = s->pointDegree[edges[edgeIdx].v];
            int und_u = s->pointUndecided[edges[edgeIdx].u];
            int und_v = s->pointUndecided[edges[edgeIdx].v];

            // Quick reject: if degree 1 with only this edge undecided
            if ((deg_u == 1 && und_u == 1) || (deg_v == 1 && und_v == 1))
                canBeOff = false;
        }

        // Test ON branch cheaply
        if (canBeOn)
        {
            if (s->pointDegree[edges[edgeIdx].u] >= 2 || s->pointDegree[edges[edgeIdx].v] >= 2)
                canBeOn = false;
        }

        // Now do full test with propagation
        auto sOff = canBeOff ? make_unique<State>(*s) : nullptr;
        auto sOn = canBeOn ? make_unique<State>(*s) : nullptr;

        if (canBeOff)
            canBeOff = applyDecision(*sOff, edgeIdx, -1) && quickValidityCheck(*sOff) && propagateConstraints(*sOff);
        if (canBeOn)
            canBeOn = applyDecision(*sOn, edgeIdx, 1) && quickValidityCheck(*sOn) && propagateConstraints(*sOn);
